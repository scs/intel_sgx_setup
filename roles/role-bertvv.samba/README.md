Attention: the Original role is role `bertvv.samba`. In the original version, passwords have to be mentioned in cleartext format.
To improve this, the following scenario was implemented:
1. a new random password is generated for every new user.
2. the role stores this password to a specific file location
3. then for each user, a samba account is generated using this password
4. a welcome message is printed upon login, asking the user to change the random password, using Motd - message of the day

Assumptions: this role takes care of creating one samba account per server user.
             the role assumes that the Linux server user accounts have been previously created
             This role doesn't deal with firewall: it assumes that the necessary ports are enabled, for more info see here: https://www.samba.org/~tpot/articles/firewall.html
